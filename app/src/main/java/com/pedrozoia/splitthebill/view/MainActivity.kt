...
import com.pedrozoia.splitthebill.adaptor.PersonAdapter
import com.pedrozoia.splitthebill.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    private fun modifyPersonList(){
        ...
        for (i in 0 until personList.size){
            totalMoneyAmount += personList[i].valorGasto.toDouble()
        }
        val dividedMoneyAmount = totalMoneyAmount / personList.size
        for (i in 0 until personList.size){
            personList[i].debito = (dividedMoneyAmount - personList[i].valorGasto.toDouble()).toString()
        }
    }
